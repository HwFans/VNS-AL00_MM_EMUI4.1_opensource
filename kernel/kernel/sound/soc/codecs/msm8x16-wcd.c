					    ...
					    uint32_t *zr)
{
	...
	if (impedance_l == impedance_l_fixed) {
		ad_logd("%s: STEREO plug type detected\n",
			 __func__);
		mbhc->hph_type = WCD_MBHC_HPH_STEREO;
	} else {
		ad_logd("%s: MONO plug type detected\n",
			__func__);
		mbhc->hph_type = WCD_MBHC_HPH_MONO;
	}
	/* Enable ZDET_CHG  */
...
exit:
	...
	ad_logd("%s: RL %d ohm, RR %d ohm\n", __func__, *zl, *zr);
skip_imped_detect:
	ad_logd("%s: Impedance detection completed\n", __func__);
}
				...
				unsigned short reg)
{
	...
	if (MSM8X16_WCD_IS_TOMBAK_REG(reg))
		...
		if (atomic_read(&pdata->mclk_enabled) == false) {
			...
			ad_logd("%s: MCLK enabled for AHB read\n", __func__);
			...
			unsigned short reg, u8 val)
{
	...
	if (MSM8X16_WCD_IS_TOMBAK_REG(reg))
		...
		mutex_lock(&pdata->cdc_mclk_mutex);
		if (atomic_read(&pdata->mclk_enabled) == false) {
			ad_logd("enable MCLK for AHB write %s:\n", __func__);
			pdata->digital_cdc_clk.clk_val = pdata->mclk_freq;
			ret = afe_set_digital_codec_core_clock(
					AFE_PORT_ID_PRIMARY_MI2S_RX,
					&pdata->digital_cdc_clk);
			...
			struct wcd9xxx_micbias_setting *micbias)
{
	...
	ret = of_property_read_u32(dev->of_node, prop_name,
			&micbias->cfilt1_mv);
	if (ret) {
		ad_dev_logd(dev, "Looking up %s property in node %s failed",
			prop_name, dev->of_node->full_name);
						...
						struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node, addr_prop_name,
			&pdata->dig_cdc_addr);
	if (ret) {
		ad_dev_logd(dev, "%s: could not find %s entry in dt\n",
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!priv) {
		ad_logd("%s: msm8x16-wcd private data is NULL\n",
			 __func__);
		return -EINVAL;
	}
	...
	mbhc = &priv->mbhc;
	if (!mbhc) {
		ad_logd("%s: mbhc not initialized\n", __func__);
		return -EINVAL;
	}

	ucontrol->value.integer.value[0] = (u32) mbhc->hph_type;
	ad_logd("%s: hph_type = %u\n", __func__, mbhc->hph_type);
					    ...
					    bool turn_on)
{
	...
	ad_dev_logd(codec->dev, "%s turn_on: %d count: %d\n", __func__, turn_on,
			count);
	if (turn_on) {
		if (!count) {
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		break;
	default:
		ad_loge("%s: event = %d not expected\n", __func__, event);
	}
	return 0;
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (!ret)
			...
			ad_dev_logd(codec->dev, "Failed to get mbhc impedance %d\n",
				ret);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec,
		...
		struct snd_soc_dai *dai)
{
	...
	if (test_bit(BUS_DOWN, &msm8x16_wcd->status_mask)) {
		ad_dev_loge(dai->codec->dev, "Error, Device is not up post SSR\n");
		return -EINVAL;
	}
	return 0;
}
...
static int msm8x16_wcd_device_up(struct snd_soc_codec *codec)
{
	...
	if (codec->reg_def_copy) {
		ad_logn("%s: Update ASOC cache", __func__);
	...
	if (ret)
		ad_dev_loge(codec->dev, "%s: mbhc initialization failed\n",__func__);
	else
...
static int msm8x16_wcd_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (strcmp(adsp_type, adsp_err) == 0) {
		ad_loge("%s: adsp subsys location not known\n", __func__);
		return -EPROBE_DEFER;
	}
	adsp_state_notifier =
...
#endif
		ad_dev_loge(codec->dev, "Failed to register %s state notifier\n",
			adsp_type);
		iounmap(msm8x16_wcd->dig_base);
		kfree(msm8x16_wcd_priv->fw_data);
		kfree(msm8x16_wcd_priv);
...
#endif
		ad_dev_logd(&spmi->dev, "Adsp is not loaded yet %d\n",
				adsp_state);
		return -EPROBE_DEFER;
	}
