	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			case AFE_CMD_ADD_TOPOLOGIES:
				atomic_set(&this_afe.state, 0);
				wake_up(&this_afe.wait[data->token]);
				ad_logd("%s: AFE_CMD_ADD_TOPOLOGIES cmd 0x%x\n",
						__func__, payload[1]);
				break;
			default:
	...
	if (ret > 0) {
		if (wait) {
			...
			if (!ret) {
				...
				ad_loge("%s: DSP returned error[%s]\n", __func__,
...
static int afe_send_custom_topology_block(struct cal_block_data *cal_block)
{
	...
	if (!cal_block) {
		ad_loge("%s: No AFE SVC cal to send!\n", __func__);
		return -EINVAL;
	}
	if (cal_block->cal_data.size <= 0) {
		ad_loge("%s: AFE SVC cal has invalid size: %zd!\n",
	...
	ad_logd("%s:cmd_id:0x%x calsize:%zd memmap_hdl:0x%x caladdr:0x%pa",
		__func__, AFE_CMD_ADD_TOPOLOGIES, cal_block->cal_data.size,
		afe_cal.mem_map_handle, &cal_block->cal_data.paddr);

	result = afe_apr_send_pkt(&afe_cal, &this_afe.wait[index]);
	if (result)
		ad_loge("%s: AFE send topology for command 0x%x failed %d\n",
...
static void afe_send_custom_topology(void)
{
	...
	if (this_afe.cal_data[cal_index] == NULL) {
		ad_loge("%s: cal_index %d not allocated!\n",
			__func__, cal_index);
		return;
	}
	...
	if (ret < 0) {
		ad_loge("%s: No cal sent for cal_index %d! ret %d\n",
			__func__, cal_index, ret);
		goto unlock;
	}
	ad_logd("%s:sent custom topology for AFE\n", __func__);
unlock:
			...
			struct cal_type_data *cal_type, u16 port_id)
{
	...
	list_for_each_safe(ptr, next,
		...
		if (afe_top->path == path) {
			ad_logd("%s: top_id:%x acdb_id:%d afe_port:%d\n",
...
static int afe_get_cal_topology_id(u16 port_id, u32 *topology_id)
{
	...
	if (this_afe.cal_data[AFE_TOPOLOGY_CAL] == NULL) {
		ad_loge("%s: [AFE_TOPOLOGY_CAL] not initialized\n", __func__);
		return -EINVAL;
	}
	if (topology_id == NULL) {
		ad_loge("%s: topology_id is NULL\n", __func__);
		return -EINVAL;
	}
	*topology_id = 0;
	...
	if (cal_block == NULL) {
		ad_loge("%s: [AFE_TOPOLOGY_CAL] not initialized for this port %d\n",
				__func__, port_id);
		ret = -EINVAL;
		goto unlock;
	}
	...
	if (!afe_top_info->topology) {
		ad_loge("%s: invalid topology id : [%d, %d]\n",
	...
	*topology_id = (u32)afe_top_info->topology;
	...
	ad_logd("%s: port_id = %u acdb_id = %d topology_id = %u ret=%d\n",
...
static int afe_send_port_topology_id(u16 port_id)
{
	...
	index = q6audio_get_port_index(port_id);
	if (index < 0 || index > AFE_MAX_PORTS) {
		ad_loge("%s: AFE port index[%d] invalid!\n",
				__func__, index);
		goto done;
	}
	...
	if (ret || !topology_id) {
		ad_logd("%s: AFE port[%d] get_cal_topology[%d] invalid!\n",
	...
	ret = afe_apr_send_pkt(&config, &this_afe.wait[index]);
	if (ret) {
		ad_loge("%s: AFE set topology id enable for port 0x%x failed %d\n",
			__func__, port_id, ret);
		goto done;
	}
done:
	ad_logd("%s: AFE set topology id 0x%x  enable for port 0x%x ret %d\n",
	...
	struct afe_param_cdc_reg_page_cfg *cdc_reg_page_cfg)
{
	...
	ret = afe_apr_send_pkt(&config, &this_afe.wait[IDX_GLOBAL_CFG]);
	if (ret)
		ad_loge("%s: AFE_PARAM_ID_CDC_REG_PAGE_CFG failed %d\n",
		       __func__, ret);
...
static int afe_aanc_port_cfg(void *apr, uint16_t tx_port, uint16_t rx_port)
{
	...
	ad_logd("%s: AANC sample rate tx rate: %d rx rate %d\n",
...
int afe_cmd_memory_map(phys_addr_t dma_addr_p, u32 dma_buf_sz)
{
	...
	if (ret < 0) {
		...
		panic("@@AFE memory map cmd failed@@");
#endif
		goto fail_cmd;
	}
		...
		u32 mem_map_handle, int bytes)
{
	...
	ad_logd("%s: buf_addr_p = 0x%pa bytes = %d\n", __func__,
						&buf_addr_p, bytes);
						...
						void *data)
{
	...
	if (cal_index == AFE_CUST_TOPOLOGY_CAL) {
		mutex_lock(&this_afe.cal_data[AFE_CUST_TOPOLOGY_CAL]->lock);
		this_afe.set_custom_topology = 1;
		ad_logd("%s:[AFE_CUSTOM_TOPOLOGY] ret = %d, cal type = %d!\n",
			__func__, ret, cal_type);
		mutex_unlock(&this_afe.cal_data[AFE_CUST_TOPOLOGY_CAL]->lock);
	}
