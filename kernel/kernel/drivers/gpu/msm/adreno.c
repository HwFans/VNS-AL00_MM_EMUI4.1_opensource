...
{
	struct kgsl_device *device = &adreno_dev->dev;
	static int display_debug_enable = 0;
	KGSL_DRV_CRIT(device, "MISC: GPU hang detected\n");
	if (!display_debug_enable){
		display_debug_enable = 1;
		dsm_client_record(gpu_dclient,"GPU HANG \n");
		dsm_client_record(gpu_dclient,"cmd:@echo 0 > /sys/kernel/debug/tracing/tracing_on");
		dsm_client_record(gpu_dclient,"@echo > /sys/kernel/debug/tracing/set_event");
		dsm_client_record(gpu_dclient,"@echo sched:sched_wait_task >> /sys/kernel/debug/tracing/set_event");
		dsm_client_record(gpu_dclient,"@echo sched:sched_process_wait >> /sys/kernel/debug/tracing/set_event");
		dsm_client_record(gpu_dclient,"@echo irq:irq_handler_entry >> /sys/kernel/debug/tracing/set_event");
		dsm_client_record(gpu_dclient,"@echo irq:irq_handler_exit >> /sys/kernel/debug/tracing/set_event");
		dsm_client_record(gpu_dclient,"@echo 1 > /sys/kernel/debug/tracing/tracing_on");
		dsm_client_record(gpu_dclient,"@echo latency-format > /sys/kernel/debug/tracing/trace_options");
		dsm_client_record(gpu_dclient,"@echo noprint-parent > /sys/kernel/debug/tracing/trace_options");
		dsm_client_record(gpu_dclient,"@echo nocontextinfo > /sys/kernel/debug/tracing/trace_options");
		dsm_client_record(gpu_dclient,"@echo verbose > /sys/kernel/debug/tracing/trace_options");
	} else {
		dsm_client_record(gpu_dclient,"cmd:@rm -rf /data/hwzd_logs/dmd_log/dmd_spcial_log/*");
		dsm_client_record(gpu_dclient,"@cp /sys/kernel/debug/tracing/trace  /data/hwzd_logs/dmd_log/dmd_spcial_log/trace");
	}
	dsm_client_notify(gpu_dclient, DSM_GPU_HANG_ERROR_NO);
