...
#define HW_CMR_LOG_TAG "sensor_otp_imx328"
...
#define IMX328_SUNNY_MODULE_VCM_OFFSET_START    100
#define IMX328_SUNNY_MODULE_VCM_OFFSET_END       100
...
static bool imx328_otp_read_id(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    CMR_LOGW("%s module info year 20%02d month %d day %d, SNO. 0x%x  vendor id&version 0x%x\n",__func__,buf[0],buf[1],buf[2],buf[3],buf[4]);
...
static bool imx328_otp_read_vcm(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (imx328_otp.vcm_start <= IMX328_SUNNY_MODULE_VCM_OFFSET_START){
		CMR_LOGE("%s, imx328_otp.vcm_start = 0x%x\n", __func__,imx328_otp.vcm_start);
		imx328_otp.vcm_start = 0;
	}
	else{
		imx328_otp.vcm_start -= IMX328_SUNNY_MODULE_VCM_OFFSET_START;
	}

	imx328_otp.vcm_end += IMX328_SUNNY_MODULE_VCM_OFFSET_END;
	...
	CMR_LOGW(" vcm starting_dac = 0x%x\n", s_ctrl->afc_otp_info.starting_dac);
	CMR_LOGW(" vcm infinity_dac = 0x%x\n", s_ctrl->afc_otp_info.starting_dac);
	CMR_LOGW(" vcm macro_dac = 0x%x\n", s_ctrl->afc_otp_info.macro_dac);
...
static bool imx328_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if(otpCheckSumVal == sum){
        ...
        CMR_LOGW("%s success, OTPSUMVAL: %d, otpCheckSumVal: %d ,sum:%d, imx328_otp_flag=0x%x\n",
 ...
 **************************************************************************
*/
int imx328_otp_func(struct msm_sensor_ctrl_t *s_ctrl, int index)
{
    int rc = 0;
    CMR_LOGI("%s enters!\n",__func__);
