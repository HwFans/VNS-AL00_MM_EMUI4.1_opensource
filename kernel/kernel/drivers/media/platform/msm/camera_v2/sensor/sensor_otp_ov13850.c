...
static  uint32_t rg_ratio_typical = 0x24D;   //the average of 4 Golden samples' RG ratio
static  uint32_t bg_ratio_typical = 0x25B;   //the average of 4 Golden samples' BG ratio
...
/****************************************************************************
* FunctionName: ov13850_otp_i2c_write;
* Description : i2c write interface;
***************************************************************************/
static int32_t ov13850_otp_i2c_write(struct msm_sensor_ctrl_t *s_ctrl,uint32_t addr, u16 data)
{
	int32_t rc = -EFAULT;
...
/****************************************************************************
* FunctionName: ov13850_otp_i2c_read;
* Description : i2c read interface;
***************************************************************************/
static int32_t ov13850_otp_i2c_read(struct msm_sensor_ctrl_t *s_ctrl, uint32_t addr, uint16_t *data)
{
	int32_t rc = -EFAULT;
...
/****************************************************************************
* FunctionName: ov13850_otp_read_block;
...
* Other       : NA;
***************************************************************************/
static bool ov13850_otp_read_block(struct msm_sensor_ctrl_t *s_ctrl, uint16_t i2c_addr, u16 reg, u8 *buf, u16 count)
{
	u16 i   = 0;
	int ret = 0;
	u16 val = 0;
	...
	for (i = 0; i < count; i++){
		ret = ov13850_otp_i2c_read(s_ctrl, (reg+i), &val);
		if (ret !=0){
...
{
	return ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR2, OTP_CHECKSUM_REG, checksum, 1);
}
...
static bool ov13850_otp_read_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR1, OTP_ID_REG, buf, 5);
...
static bool ov13850_otp_read_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR1, OTP_AWB_REG, buf, 6);
...
static void ov13850_otp_set_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(ov13850_otp.rg_ratio == 0 || ov13850_otp.bg_ratio == 0){
		CMR_LOGE("%s: rg_ratio=%d bg_ratio=%d fail\n",__func__, ov13850_otp.rg_ratio, ov13850_otp.bg_ratio);
		return;
	}
	...
	CMR_LOGD("%s R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", __func__, R_gain, B_gain, G_gain);
       /* awb set flag */
	ov13850_otp_i2c_read(s_ctrl, 0x5001, &temp);
	temp = 0x02 | temp;
	ov13850_otp_i2c_write(s_ctrl, 0x5001, temp);

	if (R_gain > 0x400){
		ov13850_otp_i2c_write(s_ctrl, 0x5056, R_gain >> 8);
		ov13850_otp_i2c_write(s_ctrl, 0x5057, R_gain & 0x00ff);
	}

	if (G_gain > 0x400){
		ov13850_otp_i2c_write(s_ctrl, 0x5058, G_gain >> 8);
		ov13850_otp_i2c_write(s_ctrl, 0x5059, G_gain & 0x00ff);
	}

	if (B_gain > 0x400){
		ov13850_otp_i2c_write(s_ctrl, 0x505a, B_gain >> 8);
		ov13850_otp_i2c_write(s_ctrl, 0x505b, B_gain & 0x00ff);
	}
...
static bool ov13850_otp_read_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR1, OTP_LSC_1_REG, ov13850_otp.lsc, 0xff-0x0b+1);
	ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR2, OTP_LSC_2_REG, &ov13850_otp.lsc[0xff-0x0b+1], 0x72+1);
	CMR_LOGD("%s LSC[0]=%x, LSC[244]=%x, LSC[245]=%x, LSC[359]=%x\n",__func__,
...
static void ov13850_otp_set_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	u16  temp = 0;
       ...
       /* lsc set flag */
	ov13850_otp_i2c_read(s_ctrl, 0x5000, &temp);
	temp = 0x01 | temp;
	ov13850_otp_i2c_write(s_ctrl, 0x5000, temp);

	for (i = 0; i < OV13850_OTP_LSC_SIZE; i++){
		ov13850_otp_i2c_write(s_ctrl, 0x5200+i, ov13850_otp.lsc[i]);
	}
...
static bool ov13850_otp_read_vcm(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ov13850_otp_read_block(s_ctrl, OTP_SLAVE_ADDR2, OTP_VCM_REG, buf, 4);
	vcm_start = buf[0];
	vcm_start <<= 8;
	vcm_start += buf[1];
	vcm_end   = buf[2];
	...
	if ((vcm_start != vcm_end ) && (vcm_end > vcm_start) && (vcm_start != 0) && (vcm_end  != 0)){
		...
		switch (ov13850_otp_vendor_module)
		{
			case LITEON_MODULE_167_NUM:
				CMR_LOGW("ov13850 otp is liteon module and product name is carmel!\n");
				vcm_offset_value = OV13850_LITEON_167_OTP_VCM_OFFSET_VALUE;
				break;
			case OFILM_MODULE_167_NUM:
				CMR_LOGW("ov13850 otp is ofilm module and product name is carmel!\n");
...
static bool ov13850_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if((otpflag & OV13850_OTP_FAIL_FLAG) == OV13850_OTP_FAIL_FLAG){
		CMR_LOGW("%s OTP data is worng, ov13850_otp_flag=0x%04x\n", __func__,otpflag);
		return false;
	...
	if ((otpCheckSumVal == 0xFF) || (false == retVal)){
		CMR_LOGW("%s OTP data has not flashed!\n", __func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal){
		CMR_LOGW("%s ov13850_otp_read_id() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal){
		CMR_LOGW("%s ov13850_otp_read_awb() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal){
		CMR_LOGW("%s ov13850_otp_read_vcm() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (otpCheckSumVal == sum){
		...
		CMR_LOGW("%s fail, OTPSUMVAL: %d, otpCheckSumVal: %d ,sum:%d \n", __func__, OTPSUMVAL, otpCheckSumVal,sum);
	}
