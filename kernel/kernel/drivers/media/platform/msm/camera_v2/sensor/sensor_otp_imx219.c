...
static int RG_Ratio_Typical = 0x021F;
static int BG_Ratio_Typical = 0x026B;
static char otp_tmp_buf[291]={0};
#define IMX219_LCS_REG_NO      280
#define IMX219_MODULE_HUAWEI_ID 0xC8 //23060200

#define IMX219_LITEON_MODULE_VENDOR_ID   0x03
#define IMX219_SUNNY_MODULE_VENDOR_ID    0x01
...
static int imx219_read_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	vendor_id = (otp_tmp_buf[4]>>4)&0x0F;
	...
	if ((IMX219_SUNNY_MODULE_VENDOR_ID == vendor_id)&&((!strncmp(s_ctrl->sensordata->sensor_name,"imx219_sunny",strlen(s_ctrl->sensordata->sensor_name)))||
		(!strncmp(s_ctrl->sensordata->sensor_name,"imx219_sunny_cml",strlen(s_ctrl->sensordata->sensor_name))))){
		CMR_LOGW("%s sunny_module\n", __func__);
		imx219_otp.module_id = SUNNY;
	} else if ((IMX219_LITEON_MODULE_VENDOR_ID == vendor_id)&&((!strncmp(s_ctrl->sensordata->sensor_name,"imx219_liteon",strlen(s_ctrl->sensordata->sensor_name)))||
		(!strncmp(s_ctrl->sensordata->sensor_name,"imx219_liteon_cml",strlen(s_ctrl->sensordata->sensor_name))))){
		CMR_LOGW("%s liteon_module\n", __func__);
		imx219_otp.module_id = LITEON;
	} else {
		CMR_LOGE("%s module_id error,vendor_id is %d and s_ctrl_name is %s .\n", 
			__func__,vendor_id,s_ctrl->sensordata->sensor_name);
		rc = -1;
		goto GET_OTP_FAIL;
	}
...
{
	module_type module;
	module = imx219_otp.module_id;
	s_ctrl->awb_otp_info.RG = imx219_otp.rg_ratio;
	s_ctrl->awb_otp_info.BG = imx219_otp.bg_ratio;
	s_ctrl->awb_otp_info.typical_RG = awb_gain_typical[module].rg_value;
	s_ctrl->awb_otp_info.typical_BG = awb_gain_typical[module].bg_value;
	return 0;
}
