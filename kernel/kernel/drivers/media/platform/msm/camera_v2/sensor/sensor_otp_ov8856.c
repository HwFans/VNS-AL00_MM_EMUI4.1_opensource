...
static int RG_Ratio_Typical = 0x268;
static int BG_Ratio_Typical = 0x2C3;

#define OV8856_MODULE_REG_NO   5
#define OV8856_AWB_REG_NO      6
#define OV8856_LCS_REG_NO      240
...
typedef struct ov8856_otp_info {
	...
	uint8_t  lsc_param[OV8856_LCS_REG_NO];
	uint8_t  checksum;
}st_ov8856_otp_info;
...
static struct msm_camera_i2c_reg_array ov8856_otp_lsc[OV8856_LCS_REG_NO];;
...
static int ov8856_otp_get_chksum(struct msm_sensor_ctrl_t* s_ctrl,enum_ov8856_groups * group_sel)
{
	...
	int rc = 0;
	rc = ov8856_otp_read_i2c(s_ctrl, 0x7207, &check_val);   //0x7207 the second otp group checksum reg
	if(rc < 0){
	...
	if(0 == check_val){
		...
		rc = ov8856_otp_read_i2c(s_ctrl, 0x710B, &check_val);   //0x710B the first otp group checksum reg
		if(rc < 0){
...
static int ov8856_otp_get_lsc(struct msm_sensor_ctrl_t* s_ctrl,enum_ov8856_groups  group_tmp)
{
	...
	rc = ov8856_otp_read_block(s_ctrl, addr, ov8856_otp.lsc_param, OV8856_LCS_REG_NO);
	if(rc < 0){
		CMR_LOGE("%s ov8856 read lsc worng!!!\n", __func__);
		return rc;
	}

	for(i = 0;i<OV8856_LCS_REG_NO;i++){
		ov8856_otp_lsc[i].reg_addr = 0x5900 + i;
...
static int ov8856_clear_otp_buffer(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for(i = 0;i<OV8856_OTP_REG_NO;i++){
		ov8856_otp_bufclear[i].reg_addr = 0x7010 + i;
...
static int ov8856_otp_init_read(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = ov8856_otp_write_i2c(s_ctrl, 0x0100, 0x01);
	if(rc < 0){
...
static int ov8856_otp_exit_read(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if( ov8856_otp_write_i2c(s_ctrl, 0x0100, 0x00) < 0 ){
...
static int ov8856_read_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	/*read lsc info*/
	ov8856_otp_get_lsc(s_ctrl,group);
	if ( rc < 0){
...
static int ov8856_update_awb_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(ov8856_otp.rg_ratio == 0 || ov8856_otp.bg_ratio == 0)
	{
	...
	if (R_gain>0x400) {
		ov8856_otp_write_i2c(s_ctrl,0x5019, R_gain>>8);
		ov8856_otp_write_i2c(s_ctrl,0x501a, R_gain & 0x00ff);
	}

	if (G_gain>0x400) {
		ov8856_otp_write_i2c(s_ctrl,0x501b, G_gain>>8);
		ov8856_otp_write_i2c(s_ctrl,0x501c, G_gain & 0x00ff);
	}

	if (B_gain>0x400) {
		ov8856_otp_write_i2c(s_ctrl,0x501d, B_gain>>8);
		ov8856_otp_write_i2c(s_ctrl,0x501e, B_gain & 0x00ff);
	}
...
static int ov8856_update_lenc_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = ov8856_otp_read_i2c(s_ctrl,0x5000, &temp);
	if(rc < 0){
		CMR_LOGE("%s: failed read 0x5000.\n", __func__);
		return rc;
	}
	temp = 0x20 | temp;
	rc = ov8856_otp_write_i2c(s_ctrl, 0x5000, temp);
	if(rc < 0){
		CMR_LOGE("%s: failed.write 0x5000\n", __func__);
		return rc;
	}
	/*0x59f8 is lens correction vscale setting register,cause horizontal band*/
	rc = ov8856_otp_write_i2c(s_ctrl, 0x59f8, 0x3d);
	if(rc < 0){
		CMR_LOGE("%s: failed.write 0x59f8\n", __func__);
		return rc;
	}
