		...
		bool enable)
{
	...
	if (enable) {
		...
		irq_status = gpio_get_value(rmi4_data->board->irq_gpio);
		tp_log_info("%s: irq status = %d \n", __func__, irq_status);
...
static int i2c_communicate_check(struct synaptics_rmi4_data *rmi4_data)
{
	...
	memset(&rmi_fd, sizeof(struct synaptics_rmi4_fn_desc), 0);
...
void synaptics_dsx_hardware_reset(struct synaptics_rmi4_data *rmi4_data)
{
	...
	enable_irq(rmi4_data->irq);
	ret = synaptics_set_glove_mode(rmi4_data, rmi4_data->glove_enabled);
	if (ret < 0) {
		tp_log_err("%s(%d)set glove mode fail, mode=%d, ret=%d\n",
			__func__, __LINE__, rmi4_data->glove_enabled, ret);
	}
