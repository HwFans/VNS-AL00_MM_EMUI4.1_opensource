...
struct fuelguage_device_ops *g_fuelguage_ops = NULL;
...
const char *g_huawei_battery_brand = "battery_brand";
static struct fuelguage_sysfs_field_info fuelguage_sysfs_field_tbl[] =
{
    ...
    FUELGUAGE_SYSFS_FIELD_RO(battery_brand_name, BATTERY_BRAND),
};
...
static void fuelguage_sysfs_init_attrs(void)
{
    ...
    fuelguage_sysfs_attrs[limit] = NULL; /* Has additional entry for this */
}
                                 ...
                                 struct device_attribute *attr, char *buf)
{
    ...
    switch(info->name)
    {
        case FUELGUAGE_SYSFS_GAUGELOG_HEAD:
        {
            ...
            ret = snprintf(buf, MAX_SIZE, "ocv        battery_type                %s",di->sysfs_data.reg_head);
            mutex_unlock(&di->sysfs_data.dump_reg_head_lock);
            return ret;
            }
        case FUELGUAGE_SYSFS_GAUGELOG:
        {
            ocv = get_loginfo_int(di->bms_psy,POWER_SUPPLY_PROP_VOLTAGE_OCV);
            get_loginfo_str(di->bms_psy,POWER_SUPPLY_PROP_BATTERY_TYPE, cType);
        ...
        case FUELGUAGE_SYSFS_BATTERY_BRAND:
        {
            return snprintf(buf, MAX_SIZE, "%s\n", g_huawei_battery_brand);
        }
        default:
        {
...
*  return value:  0-sucess or others-fail
**********************************************************/
static ssize_t fuelguage_sysfs_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    struct fuelguage_sysfs_field_info *info = NULL;
    //struct fuelguage_device_info *di = dev_get_drvdata(dev);
    long val = 0;
    ...
    switch(info->name)
    {
        case FUELGUAGE_SYSFS_TEST: /* test*/
        {
            if((strict_strtol(buf, 10, &val) < 0) || val > 10)
            {
                return -EINVAL;
            }
...
static inline void fuelguage_sysfs_remove_group(struct fuelguage_device_info *di)
{
 ...
 /**********************************************************
 *  Function:       fuelguage_ops_register
 *  Discription:    register the handler ops for fuelguageIC
 *  Parameters:   ops:operations interface of fuelguage device
 *  return value:  0-sucess or others-fail
 **********************************************************/
int fuelguage_ops_register(struct fuelguage_device_ops *fuelguage_ops)
{

    if(NULL == fuelguage_ops)
    {
        pr_err("fuelguage ops is NULL!\n");
    }

    if (g_fuelguage_ops)
    {
        pr_err("fuelguage ops have registered already\n");
        return -EBUSY;
    }
    g_fuelguage_ops = fuelguage_ops;

    return 0;
}
 /**********************************************************
 *  Function:       free_ops_info
 *  Discription:    delete the handler ops for fuelguageIC
 *  Parameters:     ops:operations interface of fuelguage device
 **********************************************************/
...
static int fuel_guage_probe(struct platform_device *pdev)
{
    ...
    np = pdev->dev.of_node;
    ret = of_property_read_string(np, "fuelguage_type", &fuelguage_type_string);
    if (ret)
    {
    ...
    di = devm_kzalloc(&pdev->dev, sizeof(struct fuelguage_device_info),GFP_KERNEL);
    if (!di)
    {
    ...
    /*is ops funtion null*/
    di->ops = g_fuelguage_ops;
    if((NULL == di->ops->dump_register)||(NULL == di->ops->dump_register))
    {
        pr_err("fuel guage ops is NULL!\n");
        goto fuel_guage_fail_1;
    }
    ...
    if(power_class)
    {
        if (NULL == fuelguage_dev){
            fuelguage_dev = device_create(power_class, NULL, 0, "%s", "coul");
        ...
        if(fuelguage_dev)
        {
            ret = sysfs_create_link(&fuelguage_dev->kobj, &di->dev->kobj, "coul_data");
            if(0 != ret)
