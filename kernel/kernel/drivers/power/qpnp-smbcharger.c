...
#include "huawei/huawei_pmi8952.h"
...
#ifdef CONFIG_HUAWEI_KERNEL
	/*
	 * there are two special cases we need to handle in function
	 * huawei_correct_usb_current (refer to function description),
	 * for these two special cases, don't need to do any other
	 * things, just return. but for other cases, we need to go
	 * through this function.
	 */
	hw_ret = huawei_correct_usb_current(chip, current_ma);
	if (!hw_ret) {
		goto out;
	}
#endif
...
static void smbchg_rerun_aicl(struct smbchg_chip *chip)
{
	...
	if ((POWER_SUPPLY_TYPE_USB == chip->usb_supply_type) ||
		hw_data.chg_adaptive_running) {
		pr_err("%s: special case, needn't to rerun aicl\n", __func__);
		return;
	}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
    if (hw_data.jeita_volt_limit) {
        hw_data.saved_vfloat_mv = vfloat_mv;
    }
#endif
...
#else
	if (!!chg_no_timer) {
		pr_err("Force disable charger timer!\n");
		enable = false;
	}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	rc = of_property_read_u32(profile_node, "qcom,vfloat-batt-warm-mv",
						&warm_voltage);
	if (rc) {
		pr_warn("couldn't find vfloat-batt-warm-mv rc=%d\n", rc);
		ret = rc;
	} else {
		hw_data.vfloat_batt_warm = warm_voltage;
	}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
    atomic_set(&loaded_batterydata, 1);
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (hw_data.ignore_usb_ibus_setting) {
		rc = chip->usb_psy->get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop);
		if ((rc == 0 && prop.intval != POWER_SUPPLY_TYPE_USB_DCP) &&
			(current_limit > SUSPEND_CURRENT_MA && current_limit < 500))
			current_limit = 500;
	}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	huawei_handle_usb_removal(chip);
#endif
...
static int smbchg_get_iusb(struct smbchg_chip *chip)
{
	...
	/* check the usb suspend bit, we will return 0 as the VCHG is floating */
				       ...
				       const union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		hw_data.chg_adaptive_running = 1;
        rc = vote(chip->usb_icl_votable, HW_AICL_VOTER,
                       !!val->intval, val->intval);
		hw_data.chg_adaptive_running = 0;
        break;
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		if ((POWER_SUPPLY_STATUS_DISCHARGING == val->intval) && (chip->batt_hot || chip->batt_cold)) {
...
#ifdef CONFIG_HUAWEI_KERNEL
		/*change the ibus current ua to mA */
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (reg & USBIN_OV_BIT)
		dsm_post_chg_bms_info(DSM_CHG_OVP_ERROR_NO, "Charger over-voltage\n");
#endif
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT)) {
		...
		if (POWER_SUPPLY_TYPE_USB == chip->usb_supply_type) {
			/*
			 * if it's weak usb case, we handle it in the function
			 * huawei_opt_weak_usb, and do not need to do anything
			 * anymore, so just return, else we need to go on and
			 * handle it in below logic
			 */
			rc = huawei_opt_weak_usb(chip);
			if (rc)
				goto out;
		}
		...
		if ((reg & ICL_MODE_MASK) != ICL_MODE_HIGH_CURRENT) {
			...
			if (rc) {
				pr_err("could not set current 150mA: %d\n", rc);
			}
#else
			rc = smbchg_hw_aicl_rerun_en(chip, false);
			if (rc)
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	if ((src_detect) && (0 == irq)&& (0 == usb_switch_get_done_flag()))
	{
		atomic_set(&usb_switch_ready, 1);
	}
	...
	if (src_detect) {
#ifdef CONFIG_HUAWEI_KERNEL
		/* Stay awake during the usb inserted */
...
static irqreturn_t aicl_done_handler(int irq, void *_chip)
{
	...
	if (chip->aicl_complete && (!chg_usb_current) &&
		(!chg_no_temp_limit)) {
    		atomic_notifier_call_chain(&hw_chg_notifier, 0, NULL);
	}
#endif
#endif
	return IRQ_HANDLED;
...
static void smbchg_hvdcp_init(struct smbchg_chip *chip)
{
	int rc = 0;

	if (chip->hvdcp_not_supported) {
		pr_info("HVDCP is not supported\n");
		rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
					HVDCP_EN_BIT, 0);
	} else {
		rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
	...
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't write hvdcp configure reg\n");
	}
}
...
#ifdef CONFIG_HUAWEI_KERNEL
			REQUEST_IRQ(chip, spmi_resource, hw_data.chg_timeout_irq,
				"chg-complete-chg-sft", huawei_chg_timeout_handler, flags, rc);
#endif
...
static int smbchg_check_chg_version(struct smbchg_chip *chip)
{
	...
	pr_smb(PR_STATUS, "pmic=%s, wa_flags=0x%x\n, hvdcp_supported=%s",
			pmic_rev_id->pmic_name, chip->wa_flags, chip->hvdcp_not_supported ? "false" : "true");
#else
	pr_smb(PR_STATUS, "pmic=%s, wa_flags=0x%x\n",
...
#ifdef CONFIG_HUAWEI_KERNEL
    /* must be called before determine initial status */
    rc = huawei_init(chip);
    if (rc) {
        dev_err(&spmi->dev,
            "Unable to initialize hardware for hw rc = %d\n", rc);
        goto free_regulator;
    }
#endif
...
#include "huawei/huawei_pmi8952.c"
