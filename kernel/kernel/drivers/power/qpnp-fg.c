...
#define MIN_BATT_VOLT_UV (3300000)
#define BOOT_EARLY_TWO_MINUTES (2*60*1000)
static unsigned long probe_jiffies = 0;
static int is_volt_good(int volt_mv) {
    int bat_cur_ma = 0, bat_temp = 0, bat_esr_mohm = 0, real_volt_mv = 0, ret = 0;

    bat_cur_ma = fg_data[FG_DATA_CURRENT].value/1000;
    bat_temp = fg_data[FG_DATA_BATT_TEMP].value/10;
    bat_esr_mohm = fg_data[FG_DATA_BATT_ESR].value/1000;
    real_volt_mv = volt_mv;
    if ((bat_cur_ma >= 0) && (bat_temp > 20) && (bat_esr_mohm <= 150) && (volt_mv >= 3100)) {//not charging,normal temp,new battery,
        unsigned long two_minutes = probe_jiffies + msecs_to_jiffies(BOOT_EARLY_TWO_MINUTES);
        if (time_is_after_jiffies(two_minutes))
            real_volt_mv = volt_mv + bat_cur_ma*bat_esr_mohm/1000;
    }

    if (real_volt_mv > MIN_BATT_VOLT_UV/1000) {
        ret = 1;
    }

    return ret;
}
static int ui_soc_val(int msoc, struct fg_chip *chip)
{
    int ui_soc = 0, volt_uv = 0;
    if(0 == msoc) {
        //update_sram_data(chip, &unused);
        volt_uv = get_sram_prop_now(chip, FG_DATA_VOLTAGE);
        if(is_input_present(chip)) {
            if (is_volt_good(volt_uv/1000)) {
                ui_soc = 1;
            } else {
                ui_soc = 0;
            }
        ...
        if (fg_debug_mask & FG_POWER_SUPPLY)
            pr_info_ratelimited("ui_soc: %d, volt_uv:%d\n", ui_soc, volt_uv);
        return ui_soc;
    }
    else if (FULL_SOC_RAW == msoc) {
        return FULL_CAPACITY;
    }
    msoc = DIV_ROUND_CLOSEST((msoc - 1) * (FULL_CAPACITY - 2), FULL_SOC_RAW - 2) + 1;
    ui_soc = (msoc+1) > FULL_CAPACITY ? FULL_CAPACITY : (msoc+1);
    return ui_soc;
}
#endif
...
static void dsm_check_vol_soc_dismatch (struct fg_chip *chip) {
    int ocv_calc_uv = 0, soc_calc = 0, soc_fg = 0;

    if ((fg_data[FG_DATA_BATT_TEMP].value >= 200) && (fg_data[FG_DATA_BATT_ESR].value/1000 <= 150)) {
        ocv_calc_uv = fg_data[FG_DATA_VOLTAGE].value + (fg_data[FG_DATA_CURRENT].value/1000)*(fg_data[FG_DATA_BATT_ESR].value/1000);
        soc_calc = lookup_soc_for_ocv(chip, ocv_calc_uv)/10;
        soc_fg = get_prop_capacity(chip);
        if (abs(soc_calc - soc_fg) > 20) {
            pr_info("vol:%d, ocv_calc:%d, soc_calc:%d, soc_fg:%d, cur:%d, esr:%d\n", \
                fg_data[FG_DATA_VOLTAGE].value, ocv_calc_uv, soc_calc, soc_fg, fg_data[FG_DATA_CURRENT].value, fg_data[FG_DATA_BATT_ESR].value);
            dsm_post_chg_bms_info(DSM_BMS_VOL_SOC_DISMATCH, "vol soc dismatch\n");
...
#ifdef CONFIG_HUAWEI_KERNEL
    dsm_check_vol_soc_dismatch(chip);
    //schedule_work(&chip->dump_sram);
#endif
	fg_mem_release(chip);
...
#else
	for (i = 0; i < SRAM_DUMP_LEN; i += 128) {
		g_sram_str[0] = '\0';
		fill_string(g_sram_str, 400, buffer + i, 128);
		pr_info("%03X %s\n", SRAM_DUMP_START + i, g_sram_str);
	}
#endif
...
static void get_batt_id_from_sram(struct fg_chip *chip) {
    int rc = 0; u8 id_info[2] = {0};
    fg_mem_lock(chip);
    rc = fg_mem_read(chip, &id_info[0], fg_data[FG_DATA_BATT_ID].address, fg_data[FG_DATA_BATT_ID].len, fg_data[FG_DATA_BATT_ID].offset, 0);
    rc |= fg_mem_read(chip, &id_info[1], fg_data[FG_DATA_BATT_ID_INFO].address, fg_data[FG_DATA_BATT_ID_INFO].len, fg_data[FG_DATA_BATT_ID_INFO].offset, 0);
    if (!rc) {
...
static int update_chg_iterm(struct fg_chip *chip);
extern const char *g_huawei_battery_brand;
#define LEAST_ID_TOLERANCE_KOHM (5)
static void check_non_std_battery(struct fg_chip *chip, struct device_node *np, const char **batt_type) {
    int id_range_pct = 0, delta = 0, limit = 0, id_sense_kohm = 0; bool std = false;
    struct property *prop = NULL; const __be32 *p = NULL; u32 id = 0; 

    if (of_property_read_u32(np, "qcom,std-id-range-pct", &id_range_pct)) {
        return;
    }
    
    id_sense_kohm = get_sram_prop_now(chip, FG_DATA_BATT_ID)/1000;
    of_property_for_each_u32(np, "qcom,std-batt-id", prop, p, id) {
        if (0 == id)
            return;
        limit = (id*id_range_pct/100) > LEAST_ID_TOLERANCE_KOHM ? (id*id_range_pct/100) : LEAST_ID_TOLERANCE_KOHM;
        delta = abs(id_sense_kohm - id);
        if (delta <= limit) {
            std = true;
            break;
        }
    ...
    if (!std) {
        char tmp_str[30] = {0};
        snprintf(tmp_str, 30, "BatId:%dkohm not std\n", id_sense_kohm);
        dsm_post_chg_bms_info(DSM_BMS_NOT_STANDARD_BATTERY, tmp_str);
        *batt_type = "default";
        pr_info("battery_type: non_std\n");
    }
    return;
}
#endif
...
wait:
...
#ifdef CONFIG_HUAWEI_KERNEL
    rc = of_property_read_u32(profile_node, "qcom,fg-iterm-ma", &settings[FG_MEM_TERM_CURRENT].value);	
    rc |= of_property_read_u32(profile_node, "qcom,fg-chg-iterm-ma", &settings[FG_MEM_CHG_TERM_CURRENT].value);	
    if (0 == rc) {
        update_iterm(chip);
	...
	if (reg & PROFILE_INTEGRITY_BIT) {
		...
		if (vbat_in_range && !fg_is_batt_empty(chip) && profiles_same) {
			...
			if (fg_est_dump)
				schedule_work(&chip->dump_sram);
			pr_info("Use default, volt_sense:%duV, volt_pred:%duV, diff_thd:%dmV\n", fg_data[FG_DATA_VOLTAGE].value,\
				fg_data[FG_DATA_CPRED_VOLTAGE].value, settings[FG_MEM_VBAT_EST_DIFF].value);
			goto done;
		}
	} else {
	...
	pr_info("Reload, volt_sense:%duV, volt_pred:%duV, profiles_same:%d\n", fg_data[FG_DATA_VOLTAGE].value,\
		fg_data[FG_DATA_CPRED_VOLTAGE].value, profiles_same);
	if (fg_est_dump)
...
#ifdef CONFIG_HUAWEI_KERNEL
	if ((0 == rc) && (NULL != strstr(saved_command_line,"androidboot.huawei_swtype=factory")))
...
#ifdef CONFIG_HUAWEI_KERNEL
    g_huawei_battery_brand = batt_type_str;
    check_non_std_battery(chip, batt_node, &g_huawei_battery_brand);
#endif
	if (fg_batt_type)
...
#ifdef CONFIG_HUAWEI_KERNEL
        dsm_post_chg_bms_info(DSM_BMS_BATT_PROFILE_EXCEPTION, "batt profile init fail\n");
#endif
    }
}
...
static int fg_of_init(struct fg_chip *chip)
{
	...
	if (data && len == THERMAL_COEFF_N_BYTES) {
		...
		pr_info("Load NTC Beta value succeed!\n");
	}
	OF_READ_SETTING(FG_MEM_RESUME_SOC, "resume-soc", rc, 1);
	if(99 == settings[FG_MEM_RESUME_SOC].value)
		settings[FG_MEM_RESUME_SOC].value = 253;
	else
...
#ifdef CONFIG_HUAWEI_KERNEL
    /* set standby_current to 50mA */
    converted_cur_raw = 0x148;
    data[0] = converted_cur_raw & 0xFF;
    data[1] = converted_cur_raw >> 8;
    rc = fg_mem_write(chip, data, 0x410, 2, 0, 0);
    if (rc)
        pr_err("failed to set standby_current rc=%d\n", rc);

    /* set slope_limiter to 5, coeff to 45, soc change .001526*225%=0.34335% per cycle */
    data[0] = 5;
    rc = fg_mem_write(chip, data, 0x430, 1, 2, 0);
    data[0] = 0xA0;
    data[1] = 0xA1;
    rc |= fg_mem_write(chip, data, 0x430, 1, 3, 0);
    rc |= fg_mem_write(chip, data+1, 0x434, 1, 0, 0);
    if (rc)
        pr_err("failed to set slope_limiter and coeff rc=%d\n", rc);
#endif
...
static void get_fuelguage_register_info(char * reg_value, u16 base_addr, char *offset, int length)
{
...
static void get_fuelguage_register_head(char * reg_head, u16 base_addr, char *offset, int length)
{
    char buff[BUFFER_SIZE] = {0};
    int i = 0;
...
{
    int cpred_vol = 0, vint_err = 0, batt_esr = 0, batt_esr_count =0, batt_soc = 0, cc_charge = 0, batt_id = 0, batt_id_info = 0, cycle_count = 0,cycle_count_id = 0, full_design = 0,charge_full = 0,charge_now =0,charge_now_raw = 0;
    ...
    snprintf(reg_value, MAX_SIZE, "%-12d %-11d %-11d %-17d %-11d %-12d %-10d %-15d %-14d %-17d %-14d %-14d %-13d %-17d ",
                cpred_vol,vint_err,batt_esr,batt_esr_count,batt_soc,cc_charge,batt_id,batt_id_info,cycle_count,cycle_count_id,full_design,charge_full,charge_now,charge_now_raw);

    /*add the four base reg item, the head is reg address*/
    get_fuelguage_register_info(reg_value, global_fg_chip->soc_base, DUMP_FG_SOC, sizeof(DUMP_FG_SOC));
    get_fuelguage_register_info(reg_value, global_fg_chip->batt_base, DUMP_FG_BATT, sizeof(DUMP_FG_BATT));
    get_fuelguage_register_info(reg_value, global_fg_chip->mem_base, DUMP_FG_MEMIF, sizeof(DUMP_FG_MEMIF));
    get_fuelguage_register_info(reg_value, global_fg_chip->tp_rev_addr, DUMP_REVID_TP_REV, sizeof(DUMP_REVID_TP_REV));
}
...
static void qpnp_get_fuelguage_register_head(char *reg_head)
{
    ...
    get_fuelguage_register_head(reg_head, global_fg_chip->soc_base, DUMP_FG_SOC, sizeof(DUMP_FG_SOC));
    get_fuelguage_register_head(reg_head, global_fg_chip->batt_base, DUMP_FG_BATT, sizeof(DUMP_FG_BATT));
    get_fuelguage_register_head(reg_head, global_fg_chip->mem_base, DUMP_FG_MEMIF, sizeof(DUMP_FG_MEMIF));
    get_fuelguage_register_head(reg_head, global_fg_chip->tp_rev_addr, DUMP_REVID_TP_REV, sizeof(DUMP_REVID_TP_REV));
}

struct fuelguage_device_ops qpnp_fuelguage_ops = {
    .dump_register = qpnp_fuelguage_dump_register,
    .get_register_head = qpnp_get_fuelguage_register_head,
};
#endif
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	u8 subtype, reg;
#ifdef CONFIG_HUAWEI_KERNEL
	struct fuelguage_device_ops *ops = NULL;
#endif
	int rc = 0;
...
#ifdef CONFIG_HUAWEI_KERNEL
	global_fg_chip = chip;
	ops = &qpnp_fuelguage_ops;
	rc = fuelguage_ops_register(ops);
	if(rc)
	{
		pr_err("fuelguage register charge ops failed!\n");
		goto power_supply_unregister;
	}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
static int fg_int_wake = 1;
static int fg_int_wake_set(const char *val, const struct kernel_param *kp) {
    int rc = 0;

    if(NULL != strstr(saved_command_line,"androidboot.huawei_swtype=factory")) {
        rc = param_set_int(val, kp);
        if (rc || (1 != fg_int_wake && 0 != fg_int_wake) || (!dbgfs_data.chip)) {
            pr_err("Set fg_int_wake fail, param error: rc: %d, val:%d\n", rc, fg_int_wake);
            return -EINVAL;
        }
        rc = irq_set_irq_wake(dbgfs_data.chip->soc_irq[DELTA_SOC].irq, fg_int_wake);
        pr_info("fg_int_wake:%d, rc:%d\n", fg_int_wake, rc);
    }
    return rc;
}
static struct kernel_param_ops fg_int_wake_ops = {
    .set = fg_int_wake_set,
    .get = param_get_int,
};
module_param_cb(fg_int_wake, &fg_int_wake_ops, &fg_int_wake, 0600);
#endif
