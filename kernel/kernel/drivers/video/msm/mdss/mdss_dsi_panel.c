...
/*set mipi status*/
	...
	if (ctrl->shutdown_sleep_enable&&is_device_reboot)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->sleep_cmds);
	if (ctrl->ds_registered) {
...
static int mdss_dsi_panel_scan_ctrl(struct mdss_panel_data *pdata,u32 scanmode)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	int ret = 0;
	if (pdata == NULL) {
	...
	switch(scanmode)
	{
		case FORWORD_SCAN: //forword scan mode
			if (ctrl_pdata->forword_scan_cmds.cmd_cnt)
				mdss_dsi_panel_cmds_send(ctrl_pdata ,&ctrl_pdata->forword_scan_cmds);
			LCD_LOG_INFO("exit %s ,forword scan mode enable = %d \n", __func__, scanmode);
			break;
		case REVERT_SCAN:// revert scan mode
			if (ctrl_pdata->revert_scan_cmds.cmd_cnt)
				mdss_dsi_panel_cmds_send(ctrl_pdata ,&ctrl_pdata->revert_scan_cmds);
			LCD_LOG_INFO("exit %s ,revert scan mode enable = %d \n", __func__, scanmode);
			break;
		default:
			ret = -EINVAL;
			LCD_LOG_ERR("%s: invalid scan mode: %d\n", __func__, scanmode);
			break;
	}
...
static int mdss_dsi_check_panel_status(struct mdss_panel_data *pdata)
{
	...
	else
	{
		if (ctrl_pdata->long_read_flag)
		{
	      ...
	      if (ctrl_pdata->reg_expect_value)
		{
...
//remove the panel-on 0A reg check function
#endif
		...
		do
		{
			...
			for(i=0;i<(ctrl->esd_cmds.cmds[j].dchdr.dlen-1);i++)
			{
				if(esd_buf[i] != ctrl->esd_cmds.cmds[j].payload[i+1])
				{
					...
					esd_buf[i] = 0xFF;//clear esd_buf data before the next read.
					break;
				}
				esd_buf[i] = 0xFF;//clear esd_buf data before the next read.
			}
...
static void mdss_panel_parse_sleep_dt(struct device_node *np,
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int rc;
	ctrl_pdata->shutdown_sleep_enable = of_property_read_bool(np, "qcom,panel-shutdown-lcd-ic-sleep-enabled");
	if(ctrl_pdata->shutdown_sleep_enable)
	{
		rc = mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->sleep_cmds,
			"qcom,panel-shutdown-lcd-ic-sleep","qcom,ic-sleep-cmds-dsi-state");
		if(rc < 0)
		{
			pr_err("%s:%d, parse esd command fail,esd check disabled\n", __func__, __LINE__);
			ctrl_pdata->shutdown_sleep_enable = false;
		}
		else
		{
			pr_info("%s:, shutdown sleep enabled \n", __func__);
		}
	}
	else
	{
		pr_info("%s:, shutdown sleep not enabled \n", __func__);
	}
}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->forword_scan_cmds,
		"qcom,panel-forword-scan-mode-cmds", "qcom,forword-scan-cmds-dsi-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->revert_scan_cmds,
		"qcom,panel-revert-scan-mode-cmds", "qcom,revert-scan-cmds-dsi-state");
	...
	ctrl_pdata->reg_expect_value = (!rc ? tmp : 0);
	...
	mdss_panel_parse_sleep_dt(np,ctrl_pdata);
#endif
	return 0;
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->panel_data.set_scan_mode = mdss_dsi_panel_scan_ctrl;
#endif
/*save ctrl_pdata */
#if defined(CONFIG_HUAWEI_KERNEL) && defined(CONFIG_DEBUG_FS)
