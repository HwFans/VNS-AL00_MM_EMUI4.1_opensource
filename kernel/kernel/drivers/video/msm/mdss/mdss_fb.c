...
static ssize_t mdss_store_scan_mode(struct device *dev,
		struct device_attribute *attr,const char *buf, size_t count)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	if((saved_command_line != NULL) && (strstr(saved_command_line, "androidboot.huawei_swtype=factory") != NULL))
	{
		if ((pdata) && (pdata->set_scan_mode)&&(mdss_fb_is_power_on(mfd)))
		{
			if(temp != mfd->panel_info->lcd_scan_mode)
			{
				ret = pdata->set_scan_mode(pdata,temp);
				if(ret)
					return ret;

				mfd->panel_info->lcd_scan_mode = temp;
			}
		}
		else
		{
...
static ssize_t mdss_show_scan_mode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	LCD_LOG_DBG("fb%d scan mode = %d\n", mfd->index, mfd->panel_info->lcd_scan_mode);
	ret = scnprintf(buf, PAGE_SIZE, "%d\n", mfd->panel_info->lcd_scan_mode);
	return ret;
}
#endif
...
static DEVICE_ATTR(lcd_scan_mode, S_IRUGO|S_IWUSR|S_IWGRP, mdss_show_scan_mode, mdss_store_scan_mode);
...
static void mdss_fb_shutdown(struct platform_device *pdev)
{
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = lcd_dbg_get_dsi_ctrl_pdata();
...
/* Add the cabl notification when display is waked up to first set backlight.*/
			sysfs_notify(&mfd->fbi->dev->kobj, NULL, "pp_bl_event");
...
#endif
			LCD_LOG_INFO("%s: backlight level = %d,set backlight time = %u,offlinecpu = %d,curfreq = %d\n",
...
static int mdss_fb_blank_unblank(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->mdp.on_fnc) {
		...
		mfd->panel_info->panel_dead = false;
			...
			mfd->panel_info->lcd_scan_mode = FORWORD_SCAN;
		#endif
