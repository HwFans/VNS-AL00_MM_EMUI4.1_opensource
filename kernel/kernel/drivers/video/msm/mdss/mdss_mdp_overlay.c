...
/* The color temprature value become invalid after reboot device.*/
...
/*modify for cabl change the unset_bl_level to 0 befor unset_bl_level used*/
...
/* Fix the conflict between loading pp_calib*.xml and setting color temprature. */
/* The color temprature value become invalid after reboot. */
static void do_color_temprature(struct msmfb_mdp_pp* p_mdp_pp)
{
	...
	switch(saved_ct_data.modify_flag){
		case CT_MODIFY_BLUE:
			...
			/*pp_calib*.xml may send MDP_PP_OPS_DISABLE, so enable it for color temprate*/
				...
				void __user *argp)
{
	...
	case mdp_op_pre_pcc_cfg:
		...
		from_pre_pcc_flag=1;  // no need to save pcc_cfg_data to origin_pcc_cfg_data
	...
	struct mdp_overlay_list *ovlist, struct mdp_overlay *ip_ovs)
{
	...
	if (sort_needed) {
		...
		if (!sorted_ovs) {
/*add release ov_lock in Abnormal case */
			mutex_unlock(&mdp5_data->ov_lock);
