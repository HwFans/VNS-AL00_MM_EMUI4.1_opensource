...
static int qseecom_load_app(struct qseecom_dev_handle *data, void __user *argp)
{
	...
	strlcpy(data->client.app_name, load_img_req.img_name,
					MAX_APP_NAME_SIZE);
	pr_crit("ss: app_id %d app_name %s pa %p\n", data->client.app_id, (char *)data->client.app_name, (void *)data->client.sb_phys);
...

{
	...
	if (((uintptr_t)req->cmd_req_buf >= 0x86C00000 &&
		((uintptr_t)req->cmd_req_buf < 0x8C600000))) {
		pr_crit("ss: Fall in modem region\n");
		BUG_ON(1);
	}
	...
	if (((uintptr_t)req->resp_buf >= 0x86C00000 &&
		((uintptr_t)req->resp_buf < 0x8C600000))) {
		pr_crit("ss: Fall in modem region\n");
		BUG_ON(1);
	}
				...
				struct qseecom_send_cmd_req *req)
{
	...
	{
		if(HUAWEI_UID != current->cred->uid)
		{
			pr_err("UID:%d from userspace is error\n",current->cred->uid);
			return -EINVAL;
		}
	}
	...
	if (qseecom.qsee_version < QSEE_VERSION_40) {
		...
		pr_crit("ss: 32bit app %s req_ptr=%#x, req_len=%x, rsp_ptr=%#x, rsp_len=%x\n",
				data->client.app_name,
				send_data_req.req_ptr,
				send_data_req.req_len,
				send_data_req.rsp_ptr,
				send_data_req.rsp_len);
	} else {
	...
	for (i = 0; i < MAX_ION_FD; i++) {
		...
		if (sg_ptr->nents == 1) {
			...
			if ((data->type == QSEECOM_CLIENT_APP &&
				...
				pr_crit("ss: App %s phy_addr=%p, len=%x\n",
						data->client.app_name,
						&(sg_dma_address(sg_ptr->sgl)),
						sg->length);
				if (((uintptr_t)&(sg_dma_address(sg_ptr->sgl)) >= 0x86C00000 &&
					((uintptr_t)&(sg_dma_address(sg_ptr->sgl)) < 0x8C600000))) {
					pr_crit("ss: Fall in modem region\n");
					BUG_ON(1);
				}
				...
				for (j = 0; j < sg_ptr->nents; j++) {
					...
					*/
					...
					pr_crit("ss: App %s phy_addr=%p, len=%x\n",
							...
							&(sg_dma_address(sg)),
							sg->length);
					if (((uintptr_t)&(sg_dma_address(sg)) >= 0x86C00000 &&
						((uintptr_t)&(sg_dma_address(sg)) < 0x8C600000))) {
						pr_crit("ss: Fall in modem region\n");
						BUG_ON(1);
					}
					pr_crit("ss: App %s phy_addr=%p, len=%x\n",
