...
static ssize_t fingerprint_chip_info_show(struct device *device, struct device_attribute *attribute, char *buf)
{
    ...
    if (fingerprint->sensor_id)
    {
        return scnprintf(buf, 50, "FPC10%s-%s\n", sensor_id, module);
    }
    else
    {
        return scnprintf(buf, 10, "N/A\n");
    }
}
...
void fingerprint_get_nav_config(struct device* dev, struct fp_data* fp_data)
{
    ...
    if(adjust1 != NAVIGATION_ADJUST_NOREVERSE && adjust1 != NAVIGATION_ADJUST_REVERSE)
    {
        ...
        fpc_log_err("navigation_adjust1 set err only support 0 and 1\n");
    }
            ...
            adjust2 != NAVIGATION_ADJUST_TURN180 && adjust2 != NAVIGATION_ADJUST_TURN270)
    {
        ...
        fpc_log_err("navigation_adjust2 set err only support 0 90 180 and 270\n");
    }

    fp_data->navigation_adjust1 = adjust1;
    fp_data->navigation_adjust2 = adjust2;

    fpc_log_info("fingerprint is %s, rotation angle is %d\n",
                  (fp_data->navigation_adjust1 == 1) ? "in front" : "on the back",
...
static int fingerprint_probe(struct spi_device* spi)
{
    ...
    if (error)
    {
        fpc_log_err("fingerprint_get_dts_data failed, error = %d\n", error);
        goto exit;
    }

    fingerprint_get_nav_config(&spi->dev, fingerprint);
    ...
    while (times_for_reset) {
        error = fingerprint_gpio_reset(fingerprint);
        if (error == 0)
            break;
    ...
    if (error)
    {
        fpc_log_err("fingerprint_gpio_reset failed, error = %d\n", error);
        goto exit;
    }
