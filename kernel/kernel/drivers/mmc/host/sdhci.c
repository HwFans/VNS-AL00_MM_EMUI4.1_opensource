...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if(!strcmp(mmc_hostname(mmc), "mmc1")){
		...
		if(unlikely(!mmc_area_cachep)) {
			EMMCSD_LOG_ERR( "sdhci: failed to create slab cache for sdcard\n");
			goto cache_fail_create;
		}
		EMMCSD_LOG_INFO("created %s cache size=%d bytes\n",mmc_hostname(mmc), max_segs_size);

		/*malloc cache for sdcard*/
		cur_sg = kmem_cache_zalloc(mmc_area_cachep, GFP_KERNEL);
		if(NULL == cur_sg){
			EMMCSD_LOG_ERR( "cur_sg cache alloc failed\n");
			goto cache_fail_create;
		}
		EMMCSD_LOG_INFO("cur cache alloc sucess\n");

		prev_sg = kmem_cache_zalloc(mmc_area_cachep, GFP_KERNEL);
		if (NULL == prev_sg){
			EMMCSD_LOG_ERR( "prev_sg cache alloc failed\n");
		...
		EMMCSD_LOG_INFO("pre cache alloc sucess\n");
	}
cache_fail_create:
#endif
	return 0;
