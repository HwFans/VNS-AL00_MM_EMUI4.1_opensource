...
static int sd_set_bus_speed_mode(struct mmc_card *card, u8 *status)
{
	...
	if ((status[16] & 0xF) != card->sd_bus_speed)
		EMMCSD_LOG_ERR("%s: Problem setting bus speed mode!\n",
			mmc_hostname(card->host));
	else {
...
int mmc_attach_sd(struct mmc_host *host)
{
	 ...
	 */
	if (ocr & 0x7F) {
		EMMCSD_LOG_INFO("%s: card claims to support voltages "
	...
	if ((ocr & MMC_VDD_165_195) &&
		...
		EMMCSD_LOG_INFO("%s: SD card claims to support the "
	...
	if (!retries) {
		EMMCSD_LOG_ERR( "%s: mmc_sd_init_card() failure (err = %d)\n",
		       mmc_hostname(host), err);
		goto err;
	}
	...
	mmc_init_clk_scaling(host);
	EMMCSD_LOG_INFO( "%s: mmc_attach_sd success\n",
		       mmc_hostname(host));
	return 0;
...
err:
	mmc_detach_bus(host);
	if (err)
		EMMCSD_LOG_ERR("%s: error %d whilst initialising SD card: rescan: %d\n",
