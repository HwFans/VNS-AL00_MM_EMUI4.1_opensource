	...
	struct mmc_ioc_cmd __user *ic_ptr)
{
    ...
    if (cmd.opcode == MMC_FFU_DOWNLOAD_OP) {
        pr_err("[emmc5.0]:%s cmd.opcode == MMC_FFU_DOWNLOAD_OP\n", __func__);
    ...
    if (cmd.opcode == MMC_FFU_INSTALL_OP) {
        pr_err("[emmc5.0]:%s cmd.opcode == MMC_FFU_INSTALL_OP\n", __func__);
			     ...
			     struct mmc_async_req *areq)
{
	...
	{
		EMMCSD_LOG_DBG("[HW]:EMMC_SANDISK_MANFID:Retry Function.");
	}
#endif
...
#endif
#ifdef CONFIG_HUAWEI_KERNEL
	MMC_FIXUP("SD16G", 0x82, 0x4a54, add_quirk_sd,
		  MMC_QUIRK_BLK_NO_CMD23),
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (mmc_card_sd(card)) {
		EMMCSD_LOG_INFO("%s: set manual_resume in mmc_blk_probe.\n", mmc_hostname(card->host));
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (mmc_card_sd(card)) {
		EMMCSD_LOG_INFO("%s: clear manual_resume and need_resume in mmc_blk_remove.\n", mmc_hostname(card->host));
...
static void mmc_blk_shutdown(struct mmc_card *card)
{
	...
	if (mmc_card_mmc(card))
	{
		...
		{
			EMMCSD_LOG_INFO("host will send power off notification to eMMC.\n");
			mmc_send_pon(card);
		}
		else
		{
			EMMCSD_LOG_INFO("host will send cmd7 and cmd5 to eMMC.\n");
